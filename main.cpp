...
int main() {
  ...
  if (!(product_type == "banana" || product_type == "apple" || product_type == "watermelon")) {
    cout << "Invalid product entered, Please enter 'banana', 'apple', or 'watermelon'." <<endl;
    return 1;
  }

  if(!(accept_sale == "y" || accept_sale == "n")) {
    cout << "Invalid promotion response, Please enter 'y' or 'n'." <<endl;
    return 1;
  }
  if (product_type == "banana" && !(unit_type == "single" || unit_type == "bunch")) {
    cout << "Invalid unit entered, for bananas Please enter 'single', or 'bunch'." <<endl;
    return 1;
  }
 if (product_type == "apple" && !(unit_type == "single" || unit_type == "bag")) {
    cout << "Invalid unit entered, for apples Please enter 'single', or 'bag'." <<endl;
    return 1;
  }
  if (product_type == "watermelon" && unit_type != "single") {
    cout << "Invalid unit entered, Please enter 'single' for watermelon" <<endl;
    return 1;
  }

// add one more unit if the user accepts promotion

if(accept_sale == "y"){
  quantity++;
}

//compute the total cost and the cost per fruit
...
if(product_type == "banana" && unit_type == "single"){
  ...
  const int bananas_per_bunch = 7;

 total_cost = quantity * price_bunch_bananas;
  item_cost = total_cost / (quantity * bananas_per_bunch);
}
else if (product_type == "apple" && unit_type == "single"){
  total_cost = quantity * (price_apple / 100.0);
  item_cost = total_cost / quantity;
}
else if (product_type == "apple" && unit_type == "bag"){
  const int apples_per_bag = 8;

 total_cost = quantity * price_bag_apples;
  item_cost = total_cost / (quantity * apples_per_bag);
}
...
//applying the 10 percent discount if promotion accepted
...
//display money values with 2 decimals
